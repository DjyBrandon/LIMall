...
type shopCartItemDiscountsType = {
...
const shopCartItemDiscounts = ref<Array<shopCartItemDiscountsType>>([])
    ...
    http.request({
        ...
        .then(({ data }) => {
            if (data.length > 0) {
                ...
                shopCartItemDiscountsParam.forEach((shopCartItemDiscount: { shopCartItems: any[] }) => {
 ...
 * +
 */
const onSelectedItem = (e: { currentTarget: { dataset: { index: number; scindex: number } } }) => {
 ...
 * 减少数量
 */
const onCountMinus = (e: { currentTarget: { dataset: { index: number; scindex: number } } }) => {
 ...
 * 增加数量
 */
const onCountPlus = (e: { currentTarget: { dataset: { index: number; scindex: number } } }) => {
    const index = e.currentTarget.dataset.index
    const scindex = e.currentTarget.dataset.scindex
    const shopCartItemDiscountsParam = shopCartItemDiscounts.value
    updateCount(shopCartItemDiscountsParam, scindex, index, 1)
 ...
 * 改变购物车数量接口
 */
const updateCount = (shopCartItemDiscountsParam: any, scindex: number, index: number, prodCount: number) => {
    uni.showLoading({
        mask: true
    })
    http.request({
